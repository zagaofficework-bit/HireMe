// ─────────────────────────────────────────────────────────────────────────
// ProfileCardCompact — matches the reference grid card
// ─────────────────────────────────────────────────────────────────────────
...
// This was rendering as "$[object Object]/hr" before — root cause of that bug.
...
export const ProfileCardCompact = ({ profile, onHire, onView }) => {
  ...
  return (
    <div className="card hover:shadow-lg transition-shadow duration-200">
      ...
      {/* Buttons — slimmer, no forced flex-stretch. Hire Now stays primary
         and a touch wider than View Profile; bookmark sits as its own
         fixed-size icon instead of squeezing into the flex row. */}
      <div className="flex items-center gap-1.5 mt-3.5">
        <button
          onClick={() => (onHire ? onHire(_id) : undefined)}
          className="flex-1 min-w-0 px-3 py-1.5 rounded-lg text-xs font-bold text-white transition-opacity hover:opacity-90"
          style={{ background: 'var(--accent)' }}
        >
          Hire Now
        </button>
        <button
          onClick={() => (onView ? onView(_id) : undefined)}
          className="flex-1 min-w-0 px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors"
          style={{ borderColor: 'var(--border)', color: 'var(--text-secondary)' }}
        >
          View Profile
        </button>
        <BookmarkButton profileId={_id} variant="icon" />
      </div>
    </div>
  );
};